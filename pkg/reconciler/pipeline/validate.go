	...
	for k, t := range udTransformers {
		...
		if transformer.Container != nil {
			if transformer.Container.Image == "" && transformer.Builtin == nil {
				return fmt.Errorf("invalid vertex %q, either specify a builtin transformer, or a customized image", k)
			}
			if transformer.Container.Image != "" && transformer.Builtin != nil {
				return fmt.Errorf("invalid vertex %q, can not specify both builtin transformer, and a customized image", k)
			}
