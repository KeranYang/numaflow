...
func TestForwarderJetStreamBuffer(t *testing.T) {
	...
	f, err := forward.NewInterStepDataForward(vertex, fromStep, toSteps, myForwardJetStreamTest{}, map[string]dfv1.OnFullWritingStrategy{"toStreamName": dfv1.RetryUntilSuccess}, myForwardJetStreamTest{}, fetchWatermark, publishWatermark)
	assert.NoError(t, err)
