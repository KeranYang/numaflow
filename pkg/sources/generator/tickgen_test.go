...
func TestRead(t *testing.T) {
	...
	mgen, err := NewMemGen(m, []isb.BufferWriter{dest}, forward.All, map[string]dfv1.OnFullWritingOption{"writer": dfv1.RetryUntilSuccess}, applier.Terminal, fetchWatermark, publishWatermark, publishWMStore)
...
func TestWatermark(t *testing.T) {
	...
	publishWMStore := store.BuildWatermarkStore(noop.NewKVNoOpStore(), noop.NewKVNoOpStore())
	mgen, err := NewMemGen(m, []isb.BufferWriter{dest}, forward.All, map[string]dfv1.OnFullWritingOption{"writer": dfv1.RetryUntilSuccess}, applier.Terminal, nil, nil, publishWMStore)
	assert.NoError(t, err)
	stop := mgen.Start()
